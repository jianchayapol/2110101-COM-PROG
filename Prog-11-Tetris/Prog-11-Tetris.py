...
def rotateR(shape):
    return  [[shape[j][i] for j in range(len(shape))][::-1] for i in range(len(shape[0]))]
    ...
    for i in range(len(board)-len(shape)+1):
        ...
        for j in range(len(shape)):
            for k in range(len(shape[0])):
                if board_c[j+i][k+c] != 0 and shape[j][k]!=0:
                    return out
                board_c[j+i][k+c] += shape[j][k]
        out.append(board_c)    
    return out
...
def animate_clear(board):
    ...
    for i in range(len(board)):
        if board_c[i].count(0) == 0:
            board_c[i]=[0 for j in range(len(board[0]))]
    out.append(board_c)
    board_d = copy.deepcopy(board_c)[::-1]
    for i in range(len(board)):
        if board_d[i].count(0) == len(board_d[0]):
            board_d.remove(board_d[i])
            board_d.append([0 for j in range(len(board[0]))])
            out.append(board_d[::-1])
    return [out[i] for i in range(len(out)) if out[i]!=out[i-1]]
